...
def main():
    ...
    gold_df = silver_df.filter(F.col("country") == F.lit("Brazil"))
    
    gold_df.writeTo("local.gold.airports_br").using("iceberg").createOrReplace()
    spark.sql("SELECT country, count(*) FROM local.gold.airports_br GROUP BY country").show()
    spark.stop()
